...
def list_folders(base_dir):
    ...
    try:
        ...
        for root, dirs, files in os.walk(base_dir, topdown=True, followlinks=False):
            # Filter directories: no dots, no cache, no app itself
            dirs[:] = [
                d
                for d in dirs
            ...
            if rel_root not in has_images_by_path:
                has_images_by_path[rel_root] = False

            # Check if current directory has any valid images
            valid_images_here = [
                f for f in files 
                if not f.startswith(".") and os.path.splitext(f)[1].lower() in IMAGE_EXTS
            ]

            if valid_images_here:
                # This folder has images, so it must be shown
                has_images_by_path[rel_root] = True
                # All its ancestors must also be shown so we can navigate to this folder
                if rel_root != "":
    ...
    # Only include folders that have images or are parents of folders with images
    folders = [
        {"path": path, "has_images": has_images_by_path[path]}
        for path in sorted(has_images_by_path.keys(), key=lambda p: p.casefold())
        if has_images_by_path[path]
    ]
